                ...
                if (def.tier === 3) {
                    ...
                    if (tier3Count >= 4) {
                        const priorities: Record<string, number> = {
                            [PlayerRole.TEAM_LEADER]: 1,
                            [PlayerRole.FRANCHISE_PLAYER]: 2,
                            [PlayerRole.CLUB_ICON]: 3,
                            [PlayerRole.LEGACY_LEGEND]: 4
                        };
                        const currentPriority = priorities[role] || 0;
                        const existing = player.roles.filter(r => this.definitions[r].tier === 3);
                        existing.sort((a, b) => (priorities[a] || 0) - (priorities[b] || 0));

                        const lowestRole = existing[0];
                        if ((priorities[lowestRole] || 0) < currentPriority) {
                            player.roles = player.roles.filter(r => r !== lowestRole);
                            removed.push(lowestRole);
                        } else {
                            return; // Cannot add
                        }
                    }
