...
export const AwardsEngine = {
    ...
    calculateSeasonAwards: (state: GameState, league: League): GalaData => {
        ...
        const pClub = playerClub || sortedClubs[0]; // Fallback
        const income = pClub.season_financials.matchday_income + pClub.season_financials.tv_income + pClub.season_financials.prize_money + pClub.season_financials.transfer_income + pClub.season_financials.sponsorship_income + pClub.season_financials.merchandise_income;
        const expenses = pClub.season_financials.wage_bill + pClub.season_financials.transfer_spend + pClub.season_financials.facility_maintenance + pClub.season_financials.scouting_costs;

        let boardScore = pClub.job_security;
        const leagueTarget = pClub.board_expectations.league_position_target;
        const leagueResult = playerPos;

        if (leagueResult < leagueTarget) boardScore = Math.min(100, boardScore + 10);
        else if (leagueResult > leagueTarget) boardScore = Math.max(0, boardScore - 10);

        let verdict = "Satisfactory";
        if (boardScore > 85) verdict = "Delighted";
        else if (boardScore > 70) verdict = "Pleased";
        else if (boardScore < 40) verdict = "Disappointed";
        else if (boardScore < 20) verdict = "Furious";

        const wonCup = pClub.trophies.some(t => t.season.includes(state.currentSeasonStartYear.toString()) && !t.name.includes(league.name));
        const cupResult = wonCup ? "Winners" : "Eliminated";

        return {
            review: {
                objectives: {
                    league: { target: `Top ${leagueTarget}`, result: `${leagueResult}${leagueResult === 1 ? 'st' : leagueResult === 2 ? 'nd' : leagueResult === 3 ? 'rd' : 'th'}`, success: leagueResult <= leagueTarget },
                    cup: { target: pClub.board_expectations.cup_target_stage.replace('_', ' '), result: cupResult, success: wonCup },
                    europe: { target: "Qualification", result: pClub.active_competitions.includes("Champions League") ? "Qualified" : "Missed", success: pClub.active_competitions.includes("Champions League") }
                },
                board_score: Math.floor(boardScore),
                board_verdict: `We are ${verdict.toLowerCase()} with your management this season.`,
                stats: {
                    played: pClub.stats.played,
                    wins: pClub.stats.won,
                    losses: pClub.stats.lost,
                    goalDiff: pClub.stats.goalsFor - pClub.stats.goalsAgainst
                },
                financials: {
                    turnover: income,
                    spend: pClub.season_financials.transfer_spend,
                    income: pClub.season_financials.transfer_income,
                    wage_bill: pClub.wage_budget_weekly,
                    budget_next: pClub.budget
                }
            },
