            ...
            if (fixture.date === state.currentDate && !fixture.played) {
                ...
                if (homeClub && awayClub) {
                    ...
                    if (homeClub.id === state.playerClubId || awayClub.id === state.playerClubId) {
                        ...
                        if (newState.manager.career_stats) {
                            newState.manager.career_stats.matches_managed++;
                            if (goalDiff > 0) newState.manager.career_stats.wins++;
                            else if (goalDiff === 0) newState.manager.career_stats.draws++;
                            else newState.manager.career_stats.losses++;

                            const milestones = [100, 250, 500, 1000];
                            if (milestones.includes(newState.manager.career_stats.matches_managed)) {
                                newNews.push({
                                    id: Date.now() + Math.random(),
                                    date: state.currentDate,
                                    headline: `MILESTONE: ${newState.manager.name} Manages ${newState.manager.career_stats.matches_managed}th Game`,
                    ...
                    if (fixture.is_knockout) {
                        ...
                        if (fixture.nextFixtureId) {
                            ...
                            if (winner.id === state.playerClubId && newState.manager.career_stats) {
                                newState.manager.career_stats.trophies_won++;
                            }
